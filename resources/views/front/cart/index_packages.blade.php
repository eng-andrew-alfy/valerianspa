...
@section('content-front')
    ...
    <div class="cart-container">
        <form id="bookingFormPackage" action="{{ route('PaymentMyFatoorah') }}" method="post">
            @csrf
            @method('POST')
            <div class="ms-form-container">
                    ...
                    <div class="ms-section-inputs">
                        <div class="flex-row gap-10 align-items-center ms-section-input waves-effect md-trigger"
                            ...
                            <div class="flex-auto" style="display:inline">
                                <a style="font-size: 16px" class="underline cursor-pointer"
                ...
                {{--                    <div class="title" style="font-weight:600; font-size: 16px;">--}}
                {{--                        {{ __('cart.services') }}--}}
                {{--                    </div>--}}
                {{--                    <div class="items">--}}
                {{--                        <div class="item">--}}
                ...
                {{--                                        {{ $Data->package->getTranslation('name', $locale) }}--}}
                {{--                                    </div>--}}
                {{--                                    <div class="more">--}}
                {{--                                        <i class="fa fa-circle"></i>--}}
                {{--                                        <div>{{ $Data->package->duration_minutes }} {{ __('messages.minutes') }}</div>--}}
                {{--                                    </div>--}}
                {{--                                </div>--}}
                {{--                                <div class="price">--}}
                {{--                                    <span class="font-bold font-16">{{ $Data->package->prices->at_home }}</span>--}}
                {{--                                    {{ __('cart.sar') }}--}}
                {{--                                </div>--}}
                {{--                            </div>--}}
                {{--                        </div>--}}
                ...
                <div class="ms-form-section">
                    <div class="ms-section-title">
                        <div class="ms-title-num">4</div>
                        <div class="ms-title-text">{{ __('cart.paymethod') }}</div>
                    </div>
                    <div class="gap-10 ms-section-inputs">
                        <div class="ms-section-input">
                            <label style="height: 50px" class="ms-radio-label">
                ...
                <div style="line-height: 30px; font-size: 16px; margin-bottom: 30px" class="ms-price-card">
                    <div class="border-bottom-1 padding-bottom-10">
                        <div class="flex-row space-between" style="display: flex; justify-content: space-between;">
                            <div class="table-title">{{ __('cart.price') }}</div>
                            <div
                                style="@if ($Data->package->discount) text-decoration-line: line-through; color: #6f6f6f @endif "
                                id="price"><span style="font-weight: bold">{{ $Data->package->prices->$price }}
                                </span> {{ __('general.sar') }}
                            </div>
                        </div>
                        @if ($Data->package->discount)
                            <div class="flex-row space-between" style="display: flex; justify-content: space-between;">
                                <div class="table-title">{{ __('cart.price_offer') }}</div>
                                <div id="price"><span
                                        style="font-weight: bold">{{ $Data->package->discount->$price - $Data->package->discount->$price * 0.15 }}
                                    </span>
                                    {{ __('general.sar') }}
                                </div>
                            </div>
                        @endif
                        <div class="flex-row space-between" style="display: flex; justify-content: space-between;">
                            <div class="table-title">{{ __('cart.discount') }}</div>
                            <div id="discount"><span style="font-weight: bold">00.00 </span> {{ __('cart.sar') }}</div>
                        </div>
                        <div class="flex-row space-between" style="display: flex; justify-content: space-between;">
                            <div class="table-title">{{ __('cart.vat') }}</div>
                            <div id="tax"><span
                                    style="font-weight: bold">{{ ($Data->package->discount ? $Data->package->discount->$price : $Data->package->prices->$price) * 0.15 }}
                                </span>
                        ...
                        <div class="flex-row space-between" style="display: none; justify-content: space-between;"
                             id="tamaraExtraInfo">
                            <div class="table-title">{{ __('cart.vat_tamara') }}</div>
                            <div id="tamara-tax"><span style="font-weight: bold"
                                                       id="texTamara">{{ ($Data->package->discount ? $Data->package->discount->$price : $Data->package->prices->$price) * 0.07 }}
                                </span> {{ __('cart.sar') }}
                            </div>
                        </div>
                    </div>
                    <div>
                        <div class="flex-row space-between"
                            ...
                            <div class="table-title">{{ __('cart.gtotal') }}</div>
                            <div style="font-weight: bold; display: flex; align-items: center;">
                                <span id="totalAmount" style="font-weight: bold">
                                    {{ $Data->package->discount ? $Data->package->discount->$price ?? '0' : $Data->package->prices->$price ?? '0' }}
                                </span>&nbsp;
                                <span style="font-weight: normal">{{ __('cart.sar') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                <div style="font-weight: bold" class="text-center">
                    <button class="ms-main-btn-submit" type="submit">{{ __('cart.complete') }}</button>
                </div>
            </div>
...
@section('scripts-front')
    ...
    {{--    <script src="{{ asset('front/dtp-datetimepicker/dtp-datetimepicker.js') }}"></script> --}}
    ...
    <script>
        $(document).ready(function () {
            ...
            function updateTotalAmount() {
                ...
                if ($('#tamaraRadio').is(':checked')) {
                    ...
                    $('#bookingFormPackage').attr('action', '{{ route('PaymentTAMARA') }}');
                } else {
                    $('#tamaraExtraInfo').css('display', 'none');
                    $('#bookingFormPackage').attr('action', '{{ route('PaymentMyFatoorah') }}');
                }
        ...
        `);
            ...
            $(document).on('click', '#apply-coupon', function () {
                ...
                const timeAvailable = $('#time-select').val();
