                ...
                <table id="lang-file" class="table table-striped table-bordered nowrap">
                    ...
                    @foreach($logs as $log)
                        <tr>
                            ...
                            <td class="text-center">{{ $log->admin->name ?? 'لم يعثر عليه' }}</td>
                            <td class="text-center">{{ $log->action }}</td>
                            <td class="text-center">{{ $log->created_at }}</td>

                            <td class="text-center">
                                <a href="{{ route('admin.logs.show', $log->id) }}" class="btn btn-info">عرض التفاصيل</a>
                            </td>
                        </tr>
                    @endforeach
...
@section('script_dashboard')
    <script>
        function showDetails(log) {
            // استخدام dd() بشكل مشابه
            console.log(log);

            // يمكنك عرض التفاصيل في تنسيق مختلف، مثل نافذة منبثقة
            alert(JSON.stringify(log, null, 2)); // عرض التفاصيل بشكل مبسط
        }
    </script>
