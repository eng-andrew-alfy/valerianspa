...
@section('page-body')
    <div class="card">
        <div class="card-block">
            <form id="addPlaceForm" action="{{ route('admin.categories.update',$category->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                    ...
                    <div class="col-sm-6 col-xl-3 m-b-30">
                        ...
                        <input type="text" name="name_en" id="name_en" class="form-control"
                               placeholder="يرجى كتابة اسم الباقة بالإنجليزية"
                    ...
                    <div class="col-sm-6 col-xl-3 m-b-30">
                        ...
                        <textarea class="form-control" name="description_ar" id="description_ar" rows="3"
                                  placeholder="الوصف بالعربية">{{  $category->getTranslation('description', 'ar') }}</textarea>
                    ...
                    <div class="col-sm-6 col-xl-3 m-b-30">
                        ...
                        <textarea class="form-control" name="description_en" id="description_en" rows="3"
                                  placeholder="الوصف بالإنجليزية">{{  $category->getTranslation('description', 'en') }}</textarea>
                ...
                <div class="row">
                    <div class="col-sm-12 col-xl-3 m-b-30">
                        ...
                        <input type="text" class="form-control" name="time"
                               ...
                               title="يرجى إدخال أرقام فقط" value="{{ $category->duration_minutes }}" required>
                        @error('time')
                        <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    {{--                               title="يرجى إدخال أرقام فقط" value="{{  $category->sessions_count  }}" required>--}}
                    {{--                        @error('num_sessions')--}}
                    {{--                        <div class="text-danger">{{ $message }}</div>--}}
                    {{--                        @enderror--}}
                    {{--                    </div>--}}
                    <div class="col-sm-12 col-xl-3 m-b-30">
                        ...
                        <input type="text" name="price_spa" class="form-control"
                               ...
                               value="{{ number_format($category->prices->at_spa, 0, '.', '') }}" required>
                        @error('price_spa')
                        <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-sm-12 col-xl-3 m-b-30">
                        ...
                        <input type="text" name="price_home" class="form-control"
                               ...
                               title="يرجى إدخال أرقام فقط"
                               value="{{ number_format($category->prices->at_home, 0, '.', '') }}" required>
                        @error('price_home')
                        <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-sm-12 col-xl-4 m-b-30">
                        ...
                        <select class="js-example-rtl col-sm-12" name="work_location[]" multiple="multiple">
                            @foreach($work_locations as $key => $label)
                                <option value="{{ $key }}"
                                        @if($category->availability->isAvailable($key)) selected @endif>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select class="js-example-rtl col-sm-12" multiple="multiple" style="width: 75%"
                            ...
                            @foreach($employees as $employee)
                                <option value="{{ $employee->id }}"
                                        @if($category->employees->contains($employee->id)) selected @endif>
                                    {{ $employee->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select class="js-example-rtl col-sm-12" name="services">
                            @foreach($services as $service)
                                <option value="{{ $service->id }}"
                                        @if($category->service_id === $service->id) selected @endif>
                ...
                <button type="submit" class="btn btn-primary" style="display: block; margin: 0 auto;">حفظ خدمة <i
